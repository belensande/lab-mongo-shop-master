  ...
  listUsers(callback = (error, users) => {}) {
    this.connect((error, database) => {
		if (error) {
			...
			database.collection(users).find({}, { _id: 0 }, callback);
    ...
    this.connect((error, database) => {
      if (error){
        ...
        // remeber once it's finish to comment callback('Error listing products');
		  database.collection(products).find({}, {_id: 0}, callback);
        ...
        // remeber once it's finish to comment callback('Error buying product');
		  database.collection(products).findOne({ "name": productName }, (error, product) => {
			  if (error) {
				  ...
				  database.collection(users).updateOne({ "firstName": userFirstName }, { $push: { "shoppingCart": product._id.toString() } }, (error, result) => {
					  if (error) {
						  callback(error);
